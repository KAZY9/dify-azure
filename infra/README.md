...
├── main.bicepparam       # パラメータ（マシン固有値は .env から読込）
├── .env.example          # infra/.env のテンプレート（.env はコミットしない）
...
マシン固有の値（SSH 許可元 IP・SSH 公開鍵）は **コミットしない `infra/.env`** に置き、`main.bicepparam` が `readEnvironmentVariable()` で読み込む（下記「マシン固有値（infra/.env）」参照）。

```bash
# 1. infra/.env を用意して実値を記入（コミットされない）
cp infra/.env.example infra/.env
#    DIFY_SSH_CIDR=<自分のIP>/32   （確認: curl -s https://api.ipify.org）
#    DIFY_SSH_PUBKEY=<cat ~/.ssh/dify_azure.pub の内容>
$EDITOR infra/.env

# 2. 環境変数として読み込む
set -a; source infra/.env; set +a

# 3. 適用前に差分を確認（-l はメタデータ保存先リージョン）
az deployment sub what-if -l japaneast -f infra/main.bicep -p infra/main.bicepparam

# 4. デプロイ（RG 作成 → RG 内に一式作成）
az deployment sub create -l japaneast -f infra/main.bicep -p infra/main.bicepparam

# 5. 出力（resourceGroupName / publicIpAddress / sshCommand / difyUrl）を確認
#    数分後、difyUrl にアクセスして初期セットアップ
```

> 注: サブスクリプションスコープのデプロイには RG 作成権限が、Key Vault のロール付与には Owner / User Access Administrator が必要。
...
公開リポジトリにコミットしたくない値（SSH 許可元 IP・SSH 公開鍵）は `infra/.env` に置き、`main.bicepparam` の `readEnvironmentVariable()` で読み込む。

- `infra/.env` は **`.gitignore` 済み**（`infra/.env.example` はテンプレートとしてコミットされる）。
- SECRET_KEY などの**本物の秘密**は param ファイルに置かず Key Vault で扱う（下記参照）。SSH 公開鍵は機密ではないが、公開リポジトリに残さない方針で env 経由にしている。
- 環境変数が未設定だと `az deployment` がビルド時に失敗する（=設定漏れを強制的に検知）。

```bash
cp infra/.env.example infra/.env   # 実値を記入
set -a; source infra/.env; set +a  # 読み込み（デプロイ前に毎回）
```
